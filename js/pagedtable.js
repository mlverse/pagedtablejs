...
var PagedTable = function (pagedTable, source) {
  ...
  var Columns = function(data, columns, options) {
    ...
    me.getWidthHtml = function() {
      /*var max = 0;
      for (var idx = page.getRowStart(); idx < page.getRowEnd(); idx++) {
        max = Math.max(max, measureHTML);
      }
      measureHTML(source.data[0])
      currentWidth = 100;*/
      return pagedTable.width;
    }

    me.getWidth = function() {
      if (source.options.html) {
        return me.getWidthHtml();
      }
      else {
        return me.getWidthText();
      }
    }
